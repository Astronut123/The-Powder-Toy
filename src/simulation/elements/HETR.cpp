...
//#TPT-Directive ElementClass Element_HETR PT_HETR 177 //You must change this line, replace HETR with your element's name and 169 with the first available id in generated/ElementClasses.h
Element_HETR::Element_HETR() //Replace all the HETR's below too
{
    Identifier = "DEFAULT_PT_HETR";				//Identifier, used for lua and internal stuff, change HETR to your element's name
    Name = "HETR";								//Name, it is recommended to use 4 letters, but less works. 5 or more will probably not fit on the buttons
    Colour = PIXPACK(0xFFBB00);					//Default color
    MenuVisible = 1;							//If it's visible in the menu, if 0 then it is hidden and can only be produced with reactions or lua
    MenuSection = SC_SPECIAL;					//The menu section it's in, see src/simulation/SimulationData.h for a list, but you can probably guess their names on your own
    ...
    Meltable = 0;								//Does it melt? 1 or higher = yes, 0 = no. This is actually only used in legacy mode, not paying attention to this property once caused a bug where many solids that don't melt would without heat sim on.
    ...
    Description = "Heats objects it touches";	//A short one sentence description of the element, shown when you mouse over it in-game.
    ...
    Graphics = &Element_HETR::graphics;
}
...
//#TPT-Directive ElementHeader Element_HETR static int graphics(GRAPHICS_FUNC_ARGS)
int Element_HETR::graphics(GRAPHICS_FUNC_ARGS)
{
	return 1;
}
